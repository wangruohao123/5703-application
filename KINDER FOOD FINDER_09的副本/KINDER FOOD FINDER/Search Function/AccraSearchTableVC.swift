...
class AccraSearchTableVC: UITableViewController , UISearchResultsUpdating{
    ...
    func searchFilter(text: String){
        searchResults = Rawdatas.filter({ (Rawdata) -> Bool in
            return Rawdata.FIELD3.localizedCaseInsensitiveContains(text) // || Rawdata.FIELD5.localizedCaseInsensitiveContains(text) || Rawdata.FIELD4.localizedCaseInsensitiveContains(text)
        })
    }
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        {
            (_, _, completion) in
            let text = "This is\(self.Rawdatas[indexPath.row].FIELD4)rate is\(self.Rawdatas[indexPath.row].FIELD5)"
